...
// Módulo Vital — Firma Tratamiento de Datos: mismo listado/tabla que /firmas (comparten
// signature_requests), filtrado del lado del backend por documentType=vital (ver
// listSignatures en signatureController.js) para no mezclarlos con las firmas antiguas
// (tratamiento de datos original + "Contrato de Activación"/Obama legado).
export default function SignatureVitalList() {
  ...
  const load = useCallback(() => {
    listSignatures({ search, status, page, limit: 15, documentType: 'vital' }).then(r => setData(r.data));
  }, [search, status, page]);
  ...
  return (
    <Layout>
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Firmas Vital</h1>
      </div>
            ...
            >
              <div className="flex items-center gap-4">
                <div>
                  ...
                  {sig.agent_name_sent && (
                    <p className="text-xs text-gray-400 mt-0.5">Agente: {sig.agent_name_sent}</p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-4">
