...
import { listCandidatos } from '../api/reclutamiento';
...
export default function CandidatoList() {
  ...
  const load = useCallback(() => {
    listCandidatos({ status, page, limit: 15 }).then(r => setData(r.data));
  }, [status, page]);
  ...
  return (
    <Layout>
      ...
      <p className="text-sm text-gray-500 mb-6">Candidatos enviados desde Hydra para firma de hoja de vida y tratamiento de datos</p>
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="divide-y divide-gray-50">
          {data.data.map(c => (
            <Link
              key={c.id}
              to={`/reclutamiento/candidatos/${c.id}`}
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">{c.candidate_email || c.candidate_phone}</p>
                </div>
              </div>
          ...
          {!data.data.length && (
            <p className="p-8 text-center text-gray-400 text-sm">No se encontraron candidatos</p>
          )}
        </div>
        {totalPages > 1 && (
