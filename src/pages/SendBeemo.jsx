...
import { sendBeemoDocument, getBeemoTemplateFields, getBeemoStats } from '../api/beemo';
import Layout from '../components/Layout';
import { Send, CheckCircle, Loader2, Mail, MessageCircle, MailPlus, Upload, FileText, X, ArrowRight, AlertTriangle } from 'lucide-react';

const CHANNELS = [
  { value: 'email',    label: 'Correo',   icon: Mail          },
  { value: 'whatsapp', label: 'WhatsApp', icon: MessageCircle },
  { value: 'both',     label: 'Ambos',    icon: MailPlus      },
];

// El backend resuelve cada dataPath con reduce() sobre segmentos separados por '.'
// (ej. "plan.compania" -> documentData.plan.compania), así que hay que anidar antes de enviar.
function nestTemplateData(flat) {
  const result = {};
...
export default function SendBeemo() {
  ...
  useEffect(() => {
    getBeemoTemplateFields().then(r => setTemplateFields(r.data.fields || [])).catch(() => {});
    getBeemoStats().then(r => setWhatsappReady(r.data.whatsappReady)).catch(() => {});
  }, []);

  const needsEmail = sendChannel === 'email' || sendChannel === 'both';
  const needsPhone = sendChannel === 'whatsapp' || sendChannel === 'both';
  const templateReady = templateFields.length > 0;
  ...
  const handleSubmit = async (e) => {
    ...
    if (source === 'upload' && !file) return setError('Sube el PDF del documento');
    if (source === 'template' && !templateReady) return setError('Plantilla pendiente de configuración');
    ...
    try {
      ...
      if (source === 'upload') fd.append('file', file);
      else fd.append('documentData', JSON.stringify(nestTemplateData(templateData)));

      const res = await sendBeemoDocument(fd);
    ...
    return (
      <Layout>
        <div className="max-w-lg mx-auto text-center py-16">
          ...
          <p className="text-gray-500 mb-1">Enviado por <strong>{ch?.label}</strong> a {recipientName}</p>
          {warning && (
            <p className="text-sm text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mt-3 inline-block">{warning}</p>
          )}
          <div className="flex gap-3 justify-center mt-6">
            ...
            <button onClick={() => navigate('/beemo/documentos')}
  ...
  return (
    <Layout>
      <div className="max-w-lg mx-auto">
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Enviar Documento Beemo</h1>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Origen del documento *</label>
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => templateReady && setSource('template')} disabled={!templateReady}
                  className={`flex flex-col items-center gap-1 py-3 px-2 rounded-xl border-2 text-xs font-medium transition-colors ${
                    source === 'template' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500'
                  } ${!templateReady ? 'opacity-50 cursor-not-allowed' : 'hover:border-gray-300'}`}>
                  Usar plantilla
                </button>
                <button type="button" onClick={() => setSource('upload')}
                  className={`flex flex-col items-center gap-1 py-3 px-2 rounded-xl border-2 text-xs font-medium transition-colors ${
                    source === 'upload' ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                  }`}>
                  Subir PDF
                </button>
              </div>
              {!templateReady && (
                <p className="text-xs text-amber-600 flex items-center gap-1 mt-2">
                  <AlertTriangle className="h-3 w-3" />Plantilla pendiente de configuración
                </p>
              )}
            </div>
            ...
            {source === 'upload' && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">PDF del documento *</label>
                {!file ? (
            ...
            {source === 'template' && templateReady && (
              <div className="space-y-3">
                {templateFields.map(field => (
                  <div key={field.dataPath}>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      {field.label || field.dataPath} {field.required && '*'}
                    </label>
                    <input
                      type={field.type === 'date' ? 'date' : 'text'}
                      value={templateData[field.dataPath] || ''}
                      onChange={e => setTemplateData(d => ({ ...d, [field.dataPath]: e.target.value }))}
            ...
            <div>
              ...
              {(sendChannel === 'whatsapp' || sendChannel === 'both') && !whatsappReady && (
                <p className="text-xs text-amber-600 flex items-center gap-1 mt-2">
                  <AlertTriangle className="h-3 w-3" />Número Beemo en proceso de aprobación
                </p>
              )}
            </div>
